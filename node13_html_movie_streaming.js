...
var server = http.createServer((request, response)=>{
	...
	var pathName = parseUrl.pathname; //    /index  /images/flower.jpg  /movie/Wildlife.mp4
	...
	if(pathName=="/index"){
		...
		fs.readFile(__dirname+pathName,function(error, imgReadData){  // d:\\ssss\fff\ddd
			if(!error){
		...
		var cnt=1;
		//2. data이벤트 생성 - 동영상파일 읽기가 되면 발생하는 이벤트
		stream.on('data',(readData)=>{
			response.write(readData);
			console.log('cnt='+ cnt++ +",  file.size=" + readData.byteLength);
		});		
		//3. end이벤트 생성 - 동영상파일을 다 읽었을대 발생하는 이벤트
		stream.on('end',()=>{
			response.end();
			console.log("stream end....................");
		});		
		//4. error이벤트 생성 - 파일을 읽기중 에러 발생시 처리할 이벤트		
